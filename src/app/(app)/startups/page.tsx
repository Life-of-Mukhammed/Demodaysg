...
import { Plus, Rocket, Users, Target, Activity, Sparkles, ArrowRight } from 'lucide-react';
...
export default async function StartupsPage() {
  ...
  const startups = await prisma.startup.findMany({
    ...
    include: { _count: { select: { members: true, listings: true, tasks: true, sprints: true } } },
  ...
  return (
    <div className="container max-w-6xl py-8 space-y-8">
      <PageHeader
        eyebrow="My Startups"
        icon={<Rocket />}
        title="Sizning startuplaringiz"
        subtitle={`${startups.length} ta startup · AI baholash, jamoa, sprint va investor readiness`}
        action={
      ...
      {startups.length === 0 ? (
        <Card className="soft-shadow">
          <CardContent className="py-20 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-6">AI sizning g'oyangizni 0-100 baholaydi va jamoa quradi</p>
            <Button asChild variant="brand" className="rounded-full"><Link href="/startups/new">Birinchi startup</Link></Button>
          </CardContent>
        </Card>
      ) : (
