...
export default function SignInPage() {
  ...
  function friendlyError(err: any) {
    ...
    if (code.includes('too-many-requests')) return 'Juda ko\'p urinish. Birozdan keyin qayta urining.';
    ...
    >
      <Card className="p-8 backdrop-blur-xl bg-card/80 border-border/50 shadow-2xl">
        <div className="text-center mb-8">
          <h1 className="font-display text-3xl font-bold">{t('signinTitle')}</h1>
        </div>
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <Button type="submit" size="lg" className="w-full" disabled={loading}>
            {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : tc('submit')}
          </Button>
        </form>
